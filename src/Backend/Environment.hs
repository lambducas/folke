    ...
    case Map.lookup name (rules env) of
        ...
        Just rule -> 
            case rule env args res of 
                ...
                Ok res_t -> 
                    if res_t == res then Ok res_t 
                    else Error TypeError ("Wrong conclusion when using rule " ++ name ++ ": expected " ++ show res_t ++ ", got " ++ show res)
...
ruleAndElimLeft _ [ArgForm _]         _ = Error TypeError "Argument needs to be an and formula."
ruleAndElimLeft _ [_]                 _ = Error TypeError "Argument needs to be a formula."
ruleAndElimLeft _ forms               _ = Error TypeError ("Rule takes 1 argument not " ++ show (List.length forms) ++ ".")

ruleAndElimRight :: Env -> [Arg] -> Formula -> Result Formula
ruleAndElimRight _ [ArgForm (And _ r)] _ = Ok r
ruleAndElimRight _ [ArgForm _]         _ = Error TypeError "Argument needs to be an and formula."
ruleAndElimRight _ [_]                 _ = Error TypeError "Argument needs to be a formula."
ruleAndElimRight _ forms               _ = Error TypeError ("Rule takes 1 argument not " ++ show (List.length forms) ++ ".")
...
ruleOrIntroLeft _ [ArgForm _]           _ = Error TypeError "Conclusion needs to be an or formula."
...
ruleOrIntroRight _ [ArgForm _]           _ = Error TypeError "Conclusion needs to be an or formula."
...
ruleOrEilm _ [ArgForm (Or a b), ArgProof (Proof [p1] c1), ArgProof (Proof [p2] c2)] _ = 
    if a == p1 then
        if b == p2 then 
...
ruleOrEilm _ [ArgForm (Or _ _), ArgProof _, _] _ = Error TypeError "Argument 3 needs to be a proof."
ruleOrEilm _ [ArgForm (Or _ _), _, _]          _ = Error TypeError "Argument 2 needs to be a proof."
ruleOrEilm _ [ArgForm _, _, _]                 _ = Error TypeError "Argument 1 needs to be an or formula."
ruleOrEilm _ [ArgForm _, _, _]                 _ = Error TypeError "Argument 1 needs to be a formula."
ruleOrEilm _ [_, _, _]                         _ = Error TypeError "Argument 1 needs to be a formula."
ruleOrEilm _ forms                             _ = Error TypeError ("Rule takes 3 arguments not " ++ show (List.length forms) ++ ".")

ruleIfIntro :: Env -> [Arg] -> Formula -> Result Formula
ruleIfIntro _ [ArgProof (Proof [a] b)] _ = Ok (If a b)
ruleIfIntro _ [_]                      _ = Error TypeError "Argument 1 needs to be a proof."
...
ruleIfEilm _ [ArgForm a, ArgForm (If b c)] r = 
    if a == b then 
        if c == r then Ok r else Error TypeError "Conclusions did not match."
    else Error TypeError "Premise did not match argument 1."
ruleIfEilm _ [ArgForm _, ArgForm _] _ = Error TypeError "Argument 2 needs to be an if-then formula."
ruleIfEilm _ [ArgForm _, _]         _ = Error TypeError "Argument 2 needs to be a formula."
ruleIfEilm _ [_        , _]         _ = Error TypeError "Argument 1 needs to be a formula."
ruleIfEilm _ forms                  _ = Error TypeError ("Rule takes 2 arguments not " ++ show (List.length forms) ++ ".")

ruleNotIntro :: Env -> [Arg] -> Formula -> Result Formula
ruleNotIntro _ [ArgProof (Proof [a] Bot)] _ = Ok (Not a)
ruleNotIntro _ [ArgProof (Proof [_] _ )]  _ = Error TypeError "Argument 1 needs to be a proof with the conclusion of bot."
ruleNotIntro _ [_]                        _ = Error TypeError "Argument 1 needs to be a proof."
...
ruleNotEilm _ [ArgForm _, ArgForm _]       _ = Error TypeError "Argument 2 needs to be a not formula."
ruleNotEilm _ [ArgForm _, _]               _ = Error TypeError "Argument 2 needs to be a formula."
ruleNotEilm _ [_, _]                       _ = Error TypeError "Argument 1 needs to be a formula."
ruleNotEilm _ forms                        _ = Error TypeError ("Rule takes 2 arguments not " ++ show (List.length forms) ++ ".")
...
ruleMT _ [ArgForm (If _ _), ArgForm _]       _ = Error TypeError "Argument 2 needs to be a not formula."
ruleMT _ [ArgForm (If _ _), _]               _ = Error TypeError "Argument 2 needs to be a formula."
ruleMT _ [ArgForm _, _ ]                     _ = Error TypeError "Argument 1 needs to be an if-then formula."
ruleMT _ [_, _]                              _ = Error TypeError "Argument 1 needs to be a formula."
ruleMT _ forms                               _ = Error TypeError ("Rule takes 2 arguments not " ++ show (List.length forms) ++ ".")

rulePBC :: Env -> [Arg] -> Formula -> Result Formula
rulePBC _ [ArgProof (Proof [Not a] Bot)] _ = Ok a
rulePBC _ [ArgProof (Proof [Not _] _)]   _ = Error TypeError "Conclusion in argument 1 needs to be a bot formula."
rulePBC _ [ArgProof (Proof [_] _)]       _ = Error TypeError "Premise in argument 1 needs to be a not formula."
rulePBC _ [_]                            _ = Error TypeError "Argument 1 needs to be a proof."
rulePBC _ forms                          _ = Error TypeError ("Rule elimination takes 1 argument not " ++ show (List.length forms) ++ ".")
...
ruleLEM _ []                _ = Error TypeError "The conclusion must be an or statement."
ruleLEM _ forms             _ = Error TypeError ("Rule takes 0 arguments not " ++ show (List.length forms) ++ ".")

ruleEqI :: Env -> [Arg] -> Formula -> Result Formula
ruleEqI _ [] r@(Eq a b) = if a == b then Ok r else Error TypeError "Left and right hand side are not the same."
ruleEqI _ [] _  = Error TypeError "The conclusion must be an eq statement."
ruleEqI _ forms _ = Error TypeError ("Rule takes 0 arguments not " ++ show (List.length forms) ++ ".")
